...
std::expected<ShaderAsset, Error> core::ShaderAsset::LoadFromMemory(
    ...
    if (memory.size() < totalSize) {
        return std::unexpected(Error::AssetParsing("Corrupted Asset: actual data size does not match header description"));
    }

    if (header.bindingCount > 0) {
        shaderAsset.bindings.resize(header.bindingCount);

        std::memcpy(shaderAsset.bindings.data(), ptr + BindingsOffset, BindingsStride * header.bindingCount);
    }
